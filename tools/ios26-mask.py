...
def alpha(img, box, fill, back, gamma=1.0):
    ...
    t = ((sub - a) @ v) / float(v @ v)
    # Отбраковка по ПЕРПЕНДИКУЛЯРНОМУ остатку. Проекции мало: любой третий цвет,
    # оказавшийся в прямоугольнике, тоже даёт ненулевую долю вдоль оси «подложка →
    # знак» и проступает в маске ровным полем. Так у знака колец фон капсулы дал
    # α ≈ 0.18 по всей площади — знак поехал в чат серым квадратом. Пиксель считается
    # материалом, только если он ЛЕЖИТ на отрезке: остаток поперёк оси мал.
    res = sub - (a + t[..., None] * v)
    off = np.sqrt((res ** 2).sum(axis=-1)) / max(float(np.sqrt(v @ v)), 1e-9)
    t = np.where(off > 0.22, 0.0, t)
    t = np.clip(t, 0.0, 1.0)
    if gamma != 1.0:
        t = t ** gamma
