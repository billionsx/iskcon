...
def check_f027() -> list:
    """ЗКН-Ф027 — У D1 ПЛАТИТСЯ ПРОСМОТРЕННАЯ СТРОКА, А НЕ ОТДАННАЯ.

    Замер 2026-08-25: 7,46 МЛРД прочитанных строк в сутки при суточном
    бесплатном пределе в 5 млн — превышение в полторы тысячи раз. Ни одного
    «тяжёлого запроса» в коде не было. Были три привычки, каждая из которых
    выглядит безобидно и каждая умножает счёт на размер таблицы.

    §А · ПОСТРАНИЧНЫЙ ОБХОД ЧЕРЕЗ OFFSET.
    `LIMIT 800 OFFSET 300000` не перепрыгивает — движок просматривает и
    выбрасывает 300 000 строк, чтобы отдать 800. Полный обход таблицы выходит
    квадратичным: N²/(2·PAGE). Дамп базы (backup.yml) стоил 96,5 млн строк
    в сутки ради 600 тысяч строк данных. Обход идёт ПО КЛЮЧУ:
    `WHERE rowid > :last ORDER BY rowid LIMIT :page`.

    §Б · ПИШЕТСЯ ТОЛЬКО ИЗМЕНИВШЕЕСЯ.
    `goswami/ingest.py` переписывал весь подтверждённый каталог на каждом
    прогоне самоцепочки: 101 389 вызовов в сутки, из них полезных — единицы.
    Перед записью состояние сверяется со снимком базы; совпало — не пишем.
    Это же лечит и запись: 3,35 млн записанных строк в сутки шли отсюда.

    §В · FTS5 СВЯЗЫВАЕТСЯ ПО rowid.
    `fts5(tid UNINDEXED, …)` означает буквально: по столбцу tid индекса НЕТ.
    Триггер `DELETE FROM katha_fts WHERE tid = old.id` искал строку перебором
    всей таблицы поиска — 6 908 прочитанных строк на КАЖДУЮ записанную.
    Правильная связь — внешнее содержимое (`content=`, `content_rowid='rowid'`):
    rowid у FTS5 индексирован всегда.

    Проверка смотрит на код, а не на счёт: счёт проверяет d1-usage.yml
    (ежедневно, падает при превышении 10% предела).
    """
    bad = []

    def strip_comments(line: str) -> str:
        """Комментарий — не код (ЗКН-Ц008). Иначе гейт ловит собственный разбор."""
        for mark in ("#", "//", "--"):
            i = line.find(mark)
            if i >= 0:
                line = line[:i]
    ...
    # §А — обход через OFFSET. Ищем ТОЛЬКО в настоящем SQL (есть SELECT и FROM):
    # слово «offset» встречается и в чужих API, и в именах полей — там оно ни при чём.
    # Собственный разбор гейта — не улика: в нём закон объясняется словами
    # и обязан содержать те самые образцы, которые он запрещает (ЗКН-Ц008).
    me = Path(__file__).resolve()
    files = [f for f in sorted(list((ROOT / "tools").rglob("*.py"))
                               + list((ROOT / "tools").rglob("*.sql"))
                               + list(CI.glob("*.yml")) + [WEB / "worker.ts"])
             if f.resolve() != me]
    for f in files:
        ...
        for i, raw in enumerate(lines, 1):
            ...
            up = line.upper()
            if "OFFSET" not in up or "SELECT" not in up or "FROM" not in up:
                continue
            # Явное исключение пишется на строке или строкой выше и всегда с причиной.
            near = " ".join(lines[max(0, i - 4):i])
            if "Ф027-исключение" in near:
                continue
            bad.append((f"{f.relative_to(ROOT)}:{i}",
                        "Ф027§А: постраничный обход через OFFSET — чтение квадратично по размеру "
                        "таблицы. Листать по ключу: WHERE rowid > :last ORDER BY rowid LIMIT :page"))

    # §В — UNINDEXED в объявлении FTS5 (тоже мимо комментариев).
    for f in files:
        code = "\n".join(strip_comments(l) for l in read(f).splitlines())
        if "UNINDEXED" in code and "USING fts5" in code:
            bad.append((str(f.relative_to(ROOT)),
                        "Ф027§В: столбец UNINDEXED в FTS5 — по нему НЕТ индекса, и любой WHERE "
                        "по нему станет перебором таблицы. Связывать по rowid (content_rowid)"))
    return bad


CHECKS = [
    ("ЗКН-Н087", "домен только из реестра routes.ts", check_n087),
    ("ЗКН-Ф027", "чтение D1: обход по ключу, запись только изменившегося", check_f027),
