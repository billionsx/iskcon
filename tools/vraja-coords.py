...
def source_alive(url: str) -> tuple:
    """ЗКН-Пл016 — ИСТОЧНИК СМЕРТЕН. ПРОВЕРЬ ЕГО, ПРЕЖДЕ ЧЕМ ВИНИТЬ СКРИПТ.
...
def main():
    ...
    for i, r in enumerate(rows):
        ...
        if lat is None:
            ...
            # ЗКН-Пл009: заполняем ТОЛЬКО пустое. Машина не перезаписывает
            # координату, поставленную человеком: у «Шриваса-анганы» и «Самадхи
            # Чанда Кази» есть исторический спор (Маяпур или город Навадвипа),
            # и позиция приложения по нему — решение человека, а не скрипта.
            d1("UPDATE tirthas SET lat = ?, lng = ? WHERE id = ? AND lat IS NULL",
