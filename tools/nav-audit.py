...
def check_n024():
    """ЗКН-Н024 — ШАПКА ВИТРИНЫ: ОДИН КОМПОНЕНТ.

    Каждая витрина рисовала шапку сама, и они разъехались: отступы 4/6/8/нет,
    у половины не было описания. Человек переключал вкладку — и текст прыгал.

    Лечится ТОЛЬКО общим компонентом: пока каждая рисует сама, любая
    договорённость разъедется снова. Витрина Богатств обязана звать HubHeader.
    """
    HUBS = {
        "AcharyaScreen.tsx": "Личности",
        "BooksHub.tsx": "Книги",
        "KirtansScreen.tsx": "Киртаны",
        "prasad/PrasadamScreen.tsx": "Прасад",
        "dhama/DhamaScreen.tsx": "Дхама",
    }
    bad = []
    for f, tab in HUBS.items():
        p = SRC / f
        if not p.exists():
            continue
...
CHECKS = [
    ("ЗКН-Н024", "шапка витрины — общий компонент", check_n024),
