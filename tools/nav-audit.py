#!/usr/bin/env python3
"""
АУДИТ НАВИГАЦИИ — домен Н (23 закона).
...
def check_n007():
    ...
    # рейка Богатств идёт ТЕМ ЖЕ порядком
    r = re.search(r'ariaLabel="Витрины Богатств"\s*\n?\s*items=\{\[(.*?)\]\}', t, re.S)
    if not r:
        bad.append(("App.tsx", "не найдена рейка «Витрины Богатств» (ЗКН-Н007)"))
    else:
        rail = re.findall(r'id:\s*"([^"]+)"', r.group(1))
        if rail != BOG_ORDER:
            bad.append(("App.tsx", "порядок рейки %s ≠ закон %s (ЗКН-Н007)" % (rail, BOG_ORDER)))
    return bad
