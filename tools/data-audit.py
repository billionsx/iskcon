...
CHECKS = [
    # ═══════════════════════════════════════════════════════════════════════════
    # ЗКН-П017 — ПРОВЕРЯЕТСЯ ОПУБЛИКОВАННАЯ КАРТОЧКА, А НЕ НАМЕРЕНИЯ СБОРКИ.
    #
    # Весь этот файл и есть его механизм: гейты бьют по D1 — по той карточке,
    # которую человек ОТКРЫВАЕТ, а не по той, которую собирался собрать скрипт.
    # Прежде П017 числился У5, а проверялся «в кузнице» (goldforge) — то есть
    # не проверялся вовсе (ЗКН-Ц012). Ровно так 15 карточек ушли в прод с лесами.
    # ═══════════════════════════════════════════════════════════════════════════
    {
        "law": "ЗКН-П015",
        "name": "стих с раздутым переводом (мусор импорта)",
        # ЗКН-Ц007 — ПРЕДМЕТ: НАСТОЯЩИЙ СТИХ, А НЕ ЛЮБАЯ СТРОКА КНИГИ.
        #
        # Порог «перевод > 2500 знаков» по ВСЕМ книгам даёт 249 «нарушений» — но 149
        # из них лежат в ПРОЗАИЧЕСКИХ книгах (письма Прабхупады, лекции, антологии),
        # где длинный текст — это глава, а не брак. Признак настоящего стиха —
        # ТРАНСЛИТЕРАЦИЯ: она есть только там, где есть шлока.
        #
        # Остаются 94 — все в «Чайтанья-бхагавате»: у стиха XVI века в поле «перевод»
        # склеено ПРИЛОЖЕНИЕ книги (ЧБ Антья 2.308 — 29 022 знака: путеводитель по
        # храмам; ЧБ Ади 17.164 — 26 312: биография автора и оглавление кханды).
        # Человек открывает стих в читалке и получает стену.
        #
        # ХРАПОВИК: чинится ПЕРЕИМПОРТОМ (`ingest-chaitanya.py` с исправленным
        # парсером), а не обрезкой на глаз — где кончается перевод и начинается
        # склейка, знает ИСТОЧНИК, а не мы. Обрезать «по смыслу» = догадка (ЗКН-БТ001).
        "sql": """SELECT COUNT(*) AS n FROM verse_texts vt JOIN verses v ON v.id = vt.verse_id
                  WHERE COALESCE(v.translit,'') <> ''
                    AND length(COALESCE(vt.translation,'')) > 2500""",
        "baseline": 94,
        "hint": "→ перевод шлоки не бывает длиной в главу. Долг 94 (ЧБ) заперт: рост запрещён (ЗКН-П015)",
    },
    {
        "law": "ЗКН-П021",
        "name": "цитата-стена (не высказывание, а простыня)",
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key='t'
                    AND t.path LIKE '%quote%' AND length(t.atom) > 1500""",
        "baseline": 24,
        "hint": "→ цитата — это ВЫСКАЗЫВАНИЕ. Долг 24 заперт храповиком (ЗКН-П021)",
    },
    {
        "law": "ЗКН-П021b",
        "name": "цитата оборвана на полуслове",
        # ЗКН-Ц007: санскритская строка законно кончается без точки — у неё в хвосте
        # стоит ДИАКРИТИКА (макрон U+0304, точка снизу U+0323, точка сверху U+0307).
        # Ссылка в скобках «[Мадхья 8.245]» — тоже законное завершение.
        # Без этого уточнения гейт насчитал бы 157 «обрывов» вместо 59 настоящих.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key='t' AND t.path LIKE '%quote%'
                    AND length(t.atom) > 80
                    AND substr(rtrim(t.atom), -1) NOT IN ('.','!','?','»','"',':',';','…',')',']','”','’')
                    AND instr(substr(t.atom, -25), char(772)) = 0
                    AND instr(substr(t.atom, -25), char(803)) = 0
                    AND instr(substr(t.atom, -25), char(775)) = 0""",
        "baseline": 59,
        "hint": "→ цитата кончается там, где кончил говорящий. Долг 59 заперт (ЗКН-П021)",
    },
    {
        "law": "ЗКН-П013",
        "name": "подпись под цитатой из НЕ-шастры",
        # «— Вриндаван дас Тхакур» под предисловием, введением или приложением —
        # это ложное свидетельство: он этого не писал.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key='by' AND t.atom IS NOT NULL
                    AND EXISTS (SELECT 1 FROM json_tree(p.longform) r
                                WHERE r.path = t.path AND r.key='ref'
                                  AND (instr(r.atom,'Предисловие') > 0 OR instr(r.atom,'Введение') > 0
                                    OR instr(r.atom,'От издател') > 0 OR instr(r.atom,'Приложение') > 0))""",
        "hint": "→ подпись ЗАРАБАТЫВАЕТСЯ уликой: предисловие и приложение — не речь автора (ЗКН-П013)",
    },
    {
        "law": "ЗКН-П014",
        "name": "анахронизм: автор подписал то, чего не мог сказать",
        # Вриндаван дас Тхакур (ум. 1589) не мог говорить о Шриле Прабхупаде.
        # Ложная подпись хуже пустоты — для библиотеки, обещающей BBT-точность.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key='by'
                    AND t.atom IN ('vrindavana-dasa-thakura','krishnadasa-kaviraja',
                                   'lochana-dasa-thakura','rupa-goswami','sanatana-goswami',
                                   'jiva-goswami','raghunatha-das-goswami')
                    AND EXISTS (SELECT 1 FROM json_tree(p.longform) q
                                WHERE q.path = t.path AND q.key='t'
                                  AND (instr(q.atom,'Прабхупад') > 0 OR instr(q.atom,'ИСККОН') > 0
                                    OR instr(q.atom,'Бхактиведанта') > 0 OR instr(q.atom,'Бхактисиддханта') > 0))""",
        "hint": "→ мог ли подписанный это сказать? Годы жизни решают (ЗКН-П014)",
    },
    {
        "law": "ЗКН-П018",
        "name": "аппарат стоит впереди смысла",
        # Машинные табы (упоминания · источники · провенанс) — это АППАРАТ к книге.
        # Он не может открывать карточку: человек пришёл за смыслом, а не за каталогом.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p
                  WHERE json_valid(p.longform)
                    AND json_array_length(json_extract(p.longform,'$.tabs')) > 1
                    AND json_extract(p.longform,'$.tabs[0].id')
                        IN ('upominaniya','istochniki','biblioteka','sources')""",
        "hint": "→ смысл вперёд, аппарат назад (ЗКН-П018)",
    },
    {
        "law": "ЗКН-П022",
        "name": "«Вклад в Гауранга Лилу» — заглушка вместо ответа",
        # ⚠️ ЗКН-Н061 · Ц007 — ГЕЙТ ЛОВИТ КЛАСС, А НЕ СТРОКУ.
        #
        # Первая редакция гейта искала БУКВАЛЬНО «Личности, с которыми он связан в
        # источниках» — форму из текста закона. А в базе заглушка пишется С ИМЕНЕМ:
        # «Личности, с которыми Медхави Дас связан в источниках». Гейт нашёл бы ноль
        # и объявил долг закрытым — при 34 живых нарушениях. Тот же капкан, что в
        # ЗКН-Н061: проверка по строке = проверки нет.
        #
        # ХРАПОВИК: чинится НАПОЛНЕНИЕМ (ПКЛ, фаза 2) — раздел должен ответить, что
        # человек дал лиле. Выдумать ответ нельзя (ЗКН-БТ001): он берётся из источника.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.atom IS NOT NULL
                    AND instr(t.atom, 'Личности, с которыми') > 0
                    AND instr(t.atom, 'в источниках') > 0""",
        "baseline": 34,
        "hint": "→ раздел отвечает: ЧТО человек дал лиле. Метаданные на месте смысла = пустота. Долг 34 заперт (ЗКН-П022)",
    },
    {
        "law": "ЗКН-П019",
    ...
    },
    {
        ...
        # ⚠️ ЗКН-Ц007 — ПРЕДМЕТ ЗАКОНА: ЦИТАТА. НЕ СПИСОК КНИГ.
        #
        # Гейт считал ЛЮБОЙ `ref` без цифры — и требовал номер стиха у того, что
        # стихом не является:
        #
        #   {"ref": "Бхагавад-гита как она есть", "to": "/bhagavad-gita"}   ← книга целиком
        #   {"ref": "Шри Шикшаштака",             "to": "/shikshashtaka"}   ← произведение
        #
        # Это БИБЛИОГРАФИЯ: кликабельная отсылка к источнику, у неё координаты нет
        # и быть не должно. Таких было 127 из 146 — то есть долг на 87% состоял из
        # того, что законом не нарушается. Предмет ЗКН-БТ003 — ЦИТАТА: узел, у
        # которого ЕСТЬ текст (`t`). Она обязана вести до стиха.
        #
        # ХРАПОВИК ДОЛГА. Выдумывать номер ЗАПРЕЩЕНО (фабрикация, ЗКН-БТ001).
        # Долг был СПРЯТАН В SQL (`COUNT(*) - 68`) — сколько его, знала только
        # формула. Теперь SQL считает честно, а признанный долг стоит в `baseline`.
        #
        # 14.07.2026: 314 → 5. Закрыто 172 ТОЧНО, без единой догадки: номер куплета
        # вычислен сверкой текста цитаты со стихом бхаджана (`prayer_verses`), а для
        # книг — со стихом в `verse_texts`. Совпадение принималось только ОДНОЗНАЧНОЕ
        # (по началу И по хвосту цитаты); где совпало несколько — не тронуто.
        # Остаток 5: 2 — источник не внесён (Падма-пурана, Калисантарана-упанишад),
        # 3 — перевод в карточке не совпал дословно ни с одним стихом книги.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key='ref'
                    AND (t.path LIKE '%quotes%' OR t.path LIKE '%cite%')
                    AND t.atom NOT GLOB '*[0-9]*'
                    AND EXISTS (SELECT 1 FROM json_tree(p.longform) x
                                WHERE x.path = t.path AND x.key='t' AND x.atom IS NOT NULL)""",
        "baseline": 5,
        "hint": "→ цитата ведёт до КОНКРЕТНОГО стиха. Долг 5 заперт храповиком: рост запрещён (ЗКН-БТ003)",
    },
    {
        "law": "ЗКН-БТ006",
