...
CHECKS = [
    ...
    {
        "law": "ЗКН-Б004",
        "name": "КАТАЛОГ КНИГ: канон имён не проверялся вовсе",
        # Гейты стерегли код и досье, а каталог книг — НЕТ. Там жили:
        # автор «Шри Шикшаштаки» = «Шри Чайтанья Махапрабху» (голая форма, ЗКН-И001),
    ...
    },
    {
        ...
        # Было 18 разорванных. Двух авторов пришлось СОЗДАТЬ — их не было в реестре
        # вовсе: Парашара Муни (отец Вьясадевы, поведал «Вишну-пурану») и Махараджа
        # Кулашекхара (один из двенадцати Альваров).
        #
        # ИСКЛЮЧЕНИЯ: тексты без единого автора (Веды, Упанишады).
        # ХРАПОВИК 2: современные биографы (Рупа-виласа дас) — им карточка личности
        # в реестре не нужна, реестр про ачарьев и спутников Господа.
        ...
        "hint": "→ связать книгу с личностью автора (ЗКН-Б005). Веды/Упанишады и современные биографы — исключение",
    },
    {
        "law": "ЗКН-Б006",
    ...
    {
        "law": "ЗКН-Б007",
        "name": "КНИГА ЧИТАЕМА В БАЗЕ, НО НЕТ В БАНДЛЕ",
        # Самая опасная из найденных: `App.tsx` делает `BOOKS[work] ? work : "bg"` —
        # если книги нет в бандле, читалка МОЛЧА открывает «Бхагавад-гиту».
        # Человек жмёт «Чайтанья-бхагавата» и попадает в другую книгу.
        # Проверяется в product-audit (нужен доступ к коду) — здесь заглушка = 0.
        "sql": "SELECT 0 AS n",
        "hint": "→ см. product-audit.py::check_b007",
    },
    {
        "law": "ЗКН-Н008",
