...
CHECKS = [
    ...
    {
        "law": "ЗКН-И003",
        "name": "«Гауранга Лила» / «Кришна Лила» — с заглавной, без дефиса",
        # Лила — не жанр и не период. Это ИГРА Господа, и пишется она как имя:
        # «Гауранга Лила», «Кришна Лила». С заглавной. Без дефиса.
        #
        # «Гаура-лила» — устаревшая форма, выведена из проекта полностью.
        #
        # ⚠️ Дефис ЗАКОНЕН внутри названий книг: «Кришна-лиламрита», «Говинда-
        # лиламрита», «Чайтанья-чаритамрита». Гейт ловит лилу как ПОНЯТИЕ, а не
        # как часть имени книги — иначе он сломает то, что править нельзя.
        "sql": """SELECT (SELECT COUNT(*) FROM entity_profiles
                          WHERE instr(COALESCE(longform,''), 'Гаура-лила') > 0)
                       + (SELECT COUNT(*) FROM entity_profiles
                          WHERE instr(COALESCE(longform,''), 'гауранга лила') > 0
                             OR instr(COALESCE(longform,''), 'кришна лила') > 0)
                       + (SELECT COUNT(*) FROM entity_profiles
                          WHERE instr(COALESCE(longform,''), 'Гауранга-лила') > 0
                             OR instr(COALESCE(longform,''), 'Гауранга-Лила') > 0)
                       AS n""",
        "hint": "→ «Гауранга Лила» / «Кришна Лила»: заглавная, без дефиса. "
                "«Гаура-лила» выведена. Дефис законен в названиях книг (ЗКН-И003)",
    },
    {
        "law": "ЗКН-Пл008",
    ...
    },
    {
        ...
        "hint": "→ стих без перевода НЕМОЙ: читатель видит толкование к тому, "
                "чего не прочёл. Искать в источнике, не переводить самому (ЗКН-Б010)",
    },
    {
        "law": "ЗКН-Сд005",
