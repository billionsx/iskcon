...
CHECKS = [
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ЗКН-Пл017 — КАТАЛОГ НЕ МЕЛЬЧАЕТ ПРИ СМЕНЕ ИСТОЧНИКА.
    #
    # 12.07.2026: страница /iskcon/centers показывала 956 центров из таблицы `places`.
    # Новый пользовательский слой (таблица `centers`, 6 seed-строк) подключили к ТОМУ
    # ЖЕ адресу — но 956 записей в него не перенесли. Локатор молча ушёл с 956 на 0:
    # «Ничего не найдено» при целой базе. Никто не заметил, потому что НИ ОДИН гейт
    # не стерёг, что публичный каталог наполнен.
    #
    # Данные не терялись — оборвался ИСТОЧНИК страницы (ср. ЗКН-Пл006). Публичный
    # каталог не имеет права падать ниже пола живых записей; смена питающей таблицы
    # требует переноса покрытия ПЕРЕД переключением. Пол 900 (было 956): падение
    # ниже = источник оборван. Проверено на живой базе (пол 2000 → n=1041, падает).
    # ═══════════════════════════════════════════════════════════════════════════
    {
        "law": "ЗКН-Пл017",
        "name": "локатор Ятры обмелел (публичный каталог центров почти пуст)",
        "sql": """SELECT CASE
                    WHEN (SELECT COUNT(*) FROM centers WHERE status='live') < 900
                    THEN 900 - (SELECT COUNT(*) FROM centers WHERE status='live')
                    ELSE 0 END AS n""",
        "baseline": 0,
        "hint": "→ /iskcon/centers держит полный каталог (было 956). Падение <900 = источник оборван, как 12.07 при переходе places→centers. Перенеси покрытие ПЕРЕД сменой источника (ЗКН-Пл017)",
    },
    {
        "law": "ЗКН-П021",
    ...
    },
    {
        ...
        #
        # ⚠️ ДЫРА, ЗАКРЫТАЯ 05.08.2026: сито ждало за аббревиатурой ЦИФРУ
        # ('ЧЧ [0-9]*'). Но у «Чайтанья-чаритамриты» и «Чайтанья-бхагаваты»
        # адрес начинается НАЗВАНИЕМ ЛИЛЫ: «ЧЧ Антья 4.227», «ЧБ Ади 17.164».
        # Восемь таких шифров лежали в карточке Дживы Госвами, и гейт молчал —
        # ровно тот же класс промаха, что ЗКН-Ц012: проверка ловила ОДНУ форму
        # записи вместо предмета. Теперь ловится и деление.
        "sql": """SELECT COUNT(*) AS n FROM entity_profiles p, json_tree(p.longform) t
                  WHERE json_valid(p.longform) AND t.key = 'ref' AND t.atom IS NOT NULL
                    AND (t.atom GLOB 'РКГД*' OR t.atom GLOB 'НП [0-9]*'
                         ...
                         OR t.atom GLOB 'ЧЧ [АМ]*' OR t.atom GLOB 'ЧБ [АМ]*'
                         OR t.atom GLOB 'ЧМ [АМ]*')""",
...
# ══════════════════════════════════════════════════════════════════════════
# РЕЖИМ ЗЕРКАЛА (ЗКН-Ф034) · 01.09.2026 — ПРИЧИНА ПАДЕНИЯ БИБЛИОТЕКИ
#
# Этот аудит держит 62 запроса `COUNT(*)` по `entity_profiles`, `verses` и
# `verse_texts` — почти все с `json_tree`, то есть ПОЛНЫМ ПРОХОДОМ таблицы:
# 63 065 строк на запрос по карточкам, 69 067 по стихам. Один прогон стоит
# около восьми миллионов прочитанных строк.
#
# Он бежит в `laws-lint`, а `laws-lint` — НА КАЖДЫЙ ПУШ. 01.09 за один час
# было пять пушей: 5 × 8 млн ≈ 43 млн строк за час, суточный предел D1
# (5 млн) вынесло, и база перестала отвечать ВСЕМ — приложение показало
# «Не удалось загрузить оглавление». Обрушил приложение не пользователь и
# не рост, а гейт качества, платящий живыми строками за проверку.
#
# Теперь при заданном `D1_MIRROR` аудит читает ночную копию содержимого:
# тот же SQL, тот же приговор, ноль прочитанных строк. Копия отстаёт от
# базы максимум на сутки — для счётного аудита это та же точность.
# ══════════════════════════════════════════════════════════════════════════
_MIRROR_CONN = None
...
def _mirror_conn():
    ...
    if _MIRROR_CONN is None:
        import sqlite3
        c = sqlite3.connect("file:%s?mode=ro" % os.environ["D1_MIRROR"], uri=True)
...
def d1(sql: str):
    if os.environ.get("D1_MIRROR"):
        try:
            # Боевой путь отдаёт ПЕРВОЕ ПОЛЕ ПЕРВОЙ СТРОКИ как число — все
            # 54 проверки суть `COUNT(*)`. Зеркало обязано отвечать той же
            # формой, иначе вызывающий сравнит список с числом и упадёт.
            rows = _mirror_conn().execute(sql).fetchall()
            return int(list(dict(rows[0]).values())[0]) if rows else 0
        except Exception as e:                              # noqa: BLE001
            # ⚠️ НЕТ ТАБЛИЦЫ В ЗЕРКАЛЕ — ЭТО КРАСНЫЙ, А НЕ ПРЕДУПРЕЖДЕНИЕ.
            #
            # Первая редакция молчала предупреждением и возвращала None, а
            # вызывающий печатал «нет доступа к D1» и шёл дальше. Итог: на
            # первом же боевом прогоне 10 законов из 54 не проверились, а
            # гейт горел ЗЕЛЁНЫМ. Зелёный гейт при непроверенных законах
            # хуже красного: он не просто не ловит нарушение, он утверждает,
            # что нарушения нет. Список таблиц зеркала ведёт ЭТОТ аудит
            # (`d1-mirror.yml`, KEEP) — значит недостача таблицы это не
            # обстоятельство, а недосмотр, и чинится он добавлением в KEEP.
            raise SystemExit(
                "::error title=Зеркало неполно::%s\n"
                "  Закон не проверен, а гейт не вправе быть зелёным при\n"
                "  непроверенном законе. Добавь таблицу в KEEP списка\n"
                "  .github/workflows/d1-mirror.yml и пересобери зеркало."
                % str(e)[:160])
...
def main():
    # Зеркало токена не требует: это файл на диске, а не сеть.
    if not os.environ.get("CLOUDFLARE_API_TOKEN") and not os.environ.get("D1_MIRROR"):
        print("АУДИТ ДАННЫХ: нет ни зеркала, ни токена — пропуск (локальный прогон)")
