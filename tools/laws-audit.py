...
def actual_level(lid: str) -> tuple[int, str]:
    ...
    # «УДАЛЕНИЕ ГЕЙТА НЕ ПРОХОДИТ МОЛЧА» механизирован ЗДЕСЬ ЖЕ: аудитор падает на
    # пропавшем гейте. Закон переехал Ц004 → Ц011 при разводе номеров — ссылка тоже.
    if lid == "ЗКН-Ц011":
    ...
    if lid == "ЗКН-Ц008":                    # инструмент не ловит объяснение
        t = (ROOT / "tools" / "laws-lint.py").read_text(encoding="utf-8")
        if "is_comment" in t:
    ...
    #
    # `surface-lint.py` ДОЛГО НЕ БЫЛ В ЭТОМ СПИСКЕ. Гейт поверхности работал,
    # стоял в деплое и ловил нарушения — а аудитор его не видел и объявлял
    # механизированные законы «голыми». Аудитор, который не знает про гейт,
    # врёт ровно так же, как закон без гейта: тихо и уверенно.
    for gate, how in (("tsc-gate.py", "гейт типов (дельта к фону)"),
                      ...
                      ("surface-lint.py", "гейт поверхности"),
