...
def check_b011():
    """ЗКН-Б011: АУДИО И ТЕКСТ — ОДНА КНИГА. Переход в ОБЕ стороны, в ТО ЖЕ место.
    ...
    if "player.track?.ref" not in t_bd:
        bad.append(("BookDetailPage.tsx",
                    "читалка не следует за звуком по ref стиха (ЗКН-Б011)"))
    return bad


def check_b010():
    """ЗКН-Б010: ПОДПИСЬ ОДНА НА ВСЕ ПОВЕРХНОСТИ.

    Читалка подписывает стих «Текст 5» / «Тексты 21-22» (мн. ч. у слитых). Плеер
    собирал строку САМ и писал «Текст 21–22» — та же книга, разные подписи. А названия
    главы в плеере не было ВОВСЕ: у ШБ дорожка — это СТИХ, и без «Песнь · Глава ·
    Название» человек не понимает, где он в книге.

    Держим два инварианта:
      • подпись стиха считается ИЗ `ref` одной функцией (`sbVerseLabel` в воркере) —
        тогда книга и плеер не могут разойтись;
      • подзаголовок дорожки — один `trackSubtitle()` на ВСЕ поверхности (мини-плеер,
        Now Playing, экран блокировки, «продолжить слушать»).
    """
    bad = []
    w = ROOT / "apps" / "web" / "worker.ts"
    if w.exists() and "sbVerseLabel" not in w.read_text(encoding="utf-8"):
        bad.append(("worker.ts", "подпись стиха не считается из ref — плеер разойдётся с книгой (ЗКН-Б010)"))
    st = SRC / "player" / "store.tsx"
    if st.exists() and "export function trackSubtitle" not in st.read_text(encoding="utf-8"):
        bad.append(("player/store.tsx", "нет ЕДИНОЙ подписи дорожки (ЗКН-Б010)"))
    for name in ("MiniPlayer.tsx", "NowPlaying.tsx"):
        f = SRC / "player" / name
        if f.exists() and "trackSubtitle(" not in f.read_text(encoding="utf-8"):
            bad.append((f"player/{name}", "поверхность собирает подпись САМА — разойдётся (ЗКН-Б010)"))
    return bad


def check_b013():
    """ЗКН-Б013: ОЧЕРЕДЬ ОПОЗНАЁТСЯ ПАРОЙ (ИСТОЧНИК, ИМЯ) — ОДНИМ ИМЕНЕМ НЕЛЬЗЯ.

    Общая очередь раздела зовётся `all` — и у киртанов, и у катхи, и у любой
    следующей аудиотеки (лекции Прабхупады на очереди). Имя УНИКАЛЬНО ВНУТРИ
    РАЗДЕЛА, а не между разделами.

    Пойман на Катхе до выпуска: `loadKirtan` держал ранний выход
    `if (bookRef.current === albumId && manifestRef.current) return;`. Человек
    стоял на Катхе (`all`), переходил в Киртаны — витрина звала `loadKirtan("all")`,
    страж видел совпадение имени, решал «уже загружено» и НЕ МЕНЯЛ манифест.
    Раздел киртанов показывал катху. Экран говорит одно, звучит другое.

    Механизм: каждый ранний выход `load*`, сверяющий `bookRef.current`, ОБЯЗАН
    сверять и `sourceRef.current`. Гейт читает store.tsx и требует этого от всех.
    """
    st = SRC / "player" / "store.tsx"
    if not st.exists():
        return [("store.tsx", "нет плеера (ЗКН-Б013)")]
    t = st.read_text(encoding="utf-8")
    bad = []
    for m in re.finditer(r"function (load[A-Z]\w*)\(([^)]*)\)\s*\{(.*?)\n  \}", t, re.S):
        name, body = m.group(1), m.group(3)
        guard = re.search(r"if\s*\([^)]*bookRef\.current[^)]*\)\s*return;", body)
        if guard and "sourceRef.current" not in guard.group(0):
            bad.append(("store.tsx", "%s: ранний выход сверяет только имя очереди — "
                                     "нужен и sourceRef.current (ЗКН-Б013)" % name))
    return bad


CHECKS = [
    ("ЗКН-Б010", "подпись одна на все поверхности", check_b010),
    ("ЗКН-Б011", "аудио ↔ текст: переход в обе стороны", check_b011),
    ...
    ("ЗКН-Б013", "очередь = (источник, имя), не имя", check_b013),
