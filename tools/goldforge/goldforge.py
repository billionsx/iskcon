...
def brief(eid, dos):
    ...
    idle = [f for f in dos["findings"]
            if f["tier"] == "strong" and f["ch"] == "k1-books-app"
            and f.get("role") == "упоминание"]
    ...
    lines = ["# Бриф куратора · %s" % eid, "",
             ...
             "## Пустые упоминания — имя есть, факта нет: %d" % len(idle), "",
    ...
    for f in idle[:60]:
        lines.append("- **%s** — %s" % (f["ref"], f["text"][:150].replace("\n", " ")))
    lines += ["", "## Кандидаты (голое имя — нужен взгляд куратора): %d" % len(cand), ""]
    for f in cand[:100]:
        lines.append("- **%s** · `%s` — %s"
