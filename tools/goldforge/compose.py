...
NARRATIVE = {"cc", "cb", "cm", "br", "ndm", "spl", "gl"}    # рассказывают жизнь
EPIC = {"sb"}                                               # писание-повествование
DOCTRINE = {"bg", "bs", "iso", "noi", "brs", "ks", "rkgd", "vp",
            "siksastaka", "manah-siksa", "mukunda-mala-stotra"}   # учение, не сюжет
TEACHING = {"prabhupada-shikshamrita"}                      # письма и наставления
POPULAR = {"owk", "rv", "pop", "bbd", "poy", "sc", "tqk", "lob"}   # книги-беседы

# ЖИТИЕ — только там, где книга РАССКАЗЫВАЕТ. «Нектар преданности, глава 11 —
# Аспекты трансцендентного служения» это доктрина, а не эпизод биографии Рупы
# Госвами: глагол в тексте есть, события — нет.
LIFE_SRC = NARRATIVE | EPIC
SHASTRA = EPIC | DOCTRINE               # чем славят: писанием, не письмом
GLORY_SRC = NARRATIVE | EPIC | DOCTRINE
...
def locator(f):
    ...
    ЗКН-БТ003 — АДРЕС ЛЕЖИТ НЕ ВСЕГДА ПЕРЕД ЗАПЯТОЙ.
    У писания ссылка «ЧЧ Мадхья 19.117, перевод»: адрес В ПЕРВОМ сегменте, за
    запятой — вид текста. У бхаджана она обратная: «„Нама-санкиртана“, куплет 4»
    — в первом сегменте НАЗВАНИЕ, а адрес за запятой. Слепой `split(",")[0]`
    оставлял название и ВЫБРАСЫВАЛ куплет, а `ref_of` дописывал книгу спереди:
    получалось «Нама-санкиртана, „Нама-санкиртана“» — ссылка без стиха
    (7 таких уехало в карточку Дживы Госвами 29.07). Поэтому: если ссылка
    ОТКРЫВАЕТСЯ названием книги, локатор — это ОСТАТОК, а не голова."""
    ref = (f.get("ref") or "").strip()
    book = (f.get("book") or "").strip()
    if book:
        for head in ("«%s»" % book, book):
            if ref.startswith(head):
                return ref[len(head):].lstrip(" ,").strip()
    ref = ref.split(",")[0]
...
def own_work_ids(hero):
    """id книг, автор которых — герой. Его слово живёт только в его книгах."""
    if not d1.available():
        return set()
    ck = ("own", hero)
    if ck not in _CACHE:
        _CACHE[ck] = {r["id"] for r in (d1.query(
            "SELECT id FROM book_catalog WHERE author_entity_id=?1", [hero]) or [])}
    return _CACHE[ck]
...
def build(dossier, hero_names, keep=None, per_work=MAX_PER_WORK):
    ...
    actor = [f for f in actor_all if f["src"] in LIFE_SRC and f["src"] not in mine]
    told = [f for f in actor_all if f["src"] in (TEACHING | POPULAR)
            and f["src"] not in mine]                # о нём рассказывает Прабхупада
    labour = pick("k1-books-app", "translation", ("труд",))
    author = [f for f in pick("k1-books-app", "translation", ("авторитет",))
              if f["src"] not in mine]
    glory = [f for f in pick("k1-books-app", "translation", ("качество",))
             if f["src"] in GLORY_SRC and f["src"] not in mine]
    ...
    # курированных. Человек открывал карточку и получал КАТАЛОГ («Житие ·
    # Лиламрита · Том 1»), а СМЫСЛ («Джаладута», «Пророчества», «Парампара»)
    # ждал его на девятой вкладке.
    #
    # Курированные главы — это КНИГА. Машинные табы — АППАРАТ к ней: источники,
    # цитаты, провенанс. Аппарат не имеет права стоять впереди книги.
    #
    # Порядок: [Вклад в Гауранга Лилу] → [главы куратора] → [аппарат машины].
    curated = []
    if keep:
        ...
        for t in keep.get("tabs", []):
            ...
            # УСТАРЕЛ, а не «сохранён» (так вчерашняя свалка переезжала под
            # видом работы куратора). Переносим ТОЛЬКО чужие, рукописные табы.
            if tid not in have and tid not in FORGE_TABS:
                curated.append(heal(t, HN, slugmap))
    if curated:
        first = [t for t in tabs if t.get("id") == "vklad-gauranga-lila"]
        rest = [t for t in tabs if t.get("id") != "vklad-gauranga-lila"]
        for t in rest:
            t["kicker"] = "ИСТОЧНИКИ · " + (t.get("kicker") or "")
        tabs = first + curated + rest
