...
def check_bt003_locator():
    """ЗКН-БТ003 — ССЫЛКА ДОВОДИТСЯ ДО СТИХА. ПРОВЕРЯЕТСЯ ВЫЗОВОМ, А НЕ РЕГУЛЯРКОЙ.

    29.07 в карточку Дживы Госвами уехали семь ссылок вида
    «Нама-санкиртана, „Нама-санкиртана“» — название дважды и НИ ОДНОГО куплета.
    Данные были правы: канал жатвы отдавал «„Нама-санкиртана“, куплет 4».
    Ломал сборщик: `locator()` брал `ref.split(",")[0]`, потому что у писания
    адрес стоит ПЕРЕД запятой («ЧЧ Мадхья 19.117, перевод»), а у бхаджана —
    ПОСЛЕ неё. Куплет выбрасывался молча, и `ref_of` дописывал книгу спереди.

    Регулярка по коду здесь бессильна: и до фикса, и после в файле стоит слово
    `split`. Поэтому гейт ЗОВЁТ функцию на обеих формах ссылки и требует, чтобы
    у бхаджана в результате остался номер, а у писания ничего не сломалось.
    Сломай `locator` обратно — этот гейт краснеет; верни — зеленеет.
    """
    sys.path.insert(0, str(ROOT / "tools"))
    try:
        from goldforge import compose
    except Exception as e:                                  # noqa: BLE001
        return [("goldforge/compose.py", "сборщик не импортируется: %s" % e)]
    bad = []
    cases = [
        ({"ref": "«Нама-санкиртана», куплет 4", "book": "Нама-санкиртана",
          "kind": "bhajan"}, "Нама-санкиртана, куплет 4"),
        ({"ref": "«Сва Нияма дашакам», куплет 9", "book": "Сва Нияма дашакам",
          "kind": "bhajan"}, "Сва Нияма дашакам, куплет 9"),
        ({"ref": "ЧЧ Мадхья 19.117", "book": "Чайтанья-чаритамрита",
          "kind": "translation"}, "Чайтанья-чаритамрита, Мадхья 19.117, перевод"),
        ({"ref": "ШБ 1.3.28", "book": "Шримад-Бхагаватам",
          "kind": "purport"}, "Шримад-Бхагаватам, 1.3.28, комментарий"),
    ]
    for fact, want in cases:
        got = compose.ref_of(fact)
        if got != want:
            bad.append(("goldforge/compose.py",
                        "ссылка «%s» → «%s», ожидалось «%s» (ЗКН-БТ003)"
                        % (fact["ref"], got, want)))
    return bad


CHECKS = [
    ("ЗКН-БТ003", "ссылка бхаджана доводится до куплета", check_bt003_locator),
