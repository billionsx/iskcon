...
# ПАЧКА ПО УМОЛЧАНИЮ ОТКЛЮЧЕНА (BATCH=1 — по файлу за вызов).
# Замер: поштучно — 100 файлов, 0 сбоев, 3,4 записи в минуту. Пачками по 8 —
# 20 залито при 68 в потерях. Причина в том, что вызов заливки атомарен на
# ВЕСЬ набор: спотыкнулся один файл — отказ приходит на всю пачку, и восемь
# скачанных файлов выбрасываются. Крупный запрос вдобавок дольше живёт и
# чаще рвётся. Код пачек оставлен: он полезен на здоровом приёмнике, но
# включать его можно только с замером, а не из общих соображений.
BATCH = int(os.getenv("BATCH") or 1)
...
WORKERS = int(os.getenv("WORKERS") or DL_PARALLEL)
