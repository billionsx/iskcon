...
SCRIPT_MARK = re.compile(
    r"[\u0300-\u036F\u0483-\u0489\u04E3\u04EF\u0100-\u017F\u1E00-\u1EFF\u00F1\u00D1]")
# «йо»/«йу»/«йе» стоят в РУССКИХ написаниях имён («Дурьйодхана») — убраны.
TRANSLIT_CYR = re.compile(r"(йа|кша|джн|шча|сйа|нйа|тйа|рйа|хйа|дхй|бхй|ттв)")
...
# NFD разбирает «й» на «и» + КРАТКУЮ. Снимая все надстрочные знаки, мы убивали
# «й»: fold("анитйам") → "анитиам". Вся детекция по «йа» была мертва.
DIA = re.compile(r"[\u0300-\u0305\u0307-\u036F\u0483-\u0489]")


def fold(w):
    w = unicodedata.normalize("NFD", w)
    w = unicodedata.normalize("NFC", DIA.sub("", w)).lower()
    return re.sub(r"[^a-zа-яё-]", "", w)
