...
ПРАВИЛО 4 — адрес замера ведёт к кадру. Имя файла, упомянутое в стандарте,
             обязано существовать в `refs/`, а НОМЕР кадра — не выходить за
             число кадров продукта по реестру. Мёртвый адрес хуже отсутствующего:
             он выглядит как доказательство. Номер вне кадра — та же смерть,
             только незаметнее: имя-то настоящее.
...
def main() -> int:
    ...
    # Имя продукта — половина адреса. Вторая половина — НОМЕР кадра, и он до сих пор
    # не проверялся: «apple_notes с.19» при восемнадцати кадрах проходило молча.
    # Такой адрес хуже отсутствующего ровно тем же — выглядит как доказательство.
    counts = {m[0]: int(m[1]) for m in re.findall(
        r"\|\s*\d+\s*\|\s*`?(apple_[a-z0-9_]+)\.pdf`?\s*\|[^|]*\|\s*(\d+)\s*\|", index_txt)}
    for stem, nums in re.findall(r"`?(apple_[a-z0-9_]+)`?\s+с\.\s*(\d+(?:\s*,\s*\d+)*)", std):
        total = counts.get(stem)
        if not total:
            continue
        for n in (int(x) for x in nums.split(",")):
            if n < 1 or n > total:
                bad.append(f"docs/design/ios26/STANDARD_ios26_css.md — ЗКН-Д026: адрес "
                           f"«{stem} с.{n}», а у продукта кадров всего {total}; "
                           f"номер вне кадра — тот же мёртвый адрес, только незаметнее")
