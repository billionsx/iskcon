...
def check_k001():
    """ЗКН-К001 — ОБЛОЖКА-СЛАЙДЕР: ОДИН МОДУЛЬ. СВОЯ РЕАЛИЗАЦИЯ ЗАПРЕЩЕНА.

    Обложка с перелистыванием — не «повертеть картинки». Это жесты, инерция,
    точки-индикаторы, предзагрузка соседнего кадра, поведение на границах,
    доступность с клавиатуры. Каждый, кто напишет своё, напишет ХУЖЕ — и по-своему.

    Тогда в приложении заведётся ДВА разных перелистывания: где-то тянется
    плавно, где-то дёргается; где-то есть точки, где-то нет. Человек не поймёт,
    что это одно и то же действие.

    Модуль один: `CardCover` (`useCoverSlider`).
    """
    import re as _re
    bad = []
    for fp in sorted(SRC.rglob("*.tsx")):
        if "CardCover" in fp.name:
            continue
        t = fp.read_text(encoding="utf-8")
        if "CardCover" in t or "useCoverSlider" in t:
            continue
        if _re.search(r'(?:covers|heroes)\[', t) and _re.search(r'set(?:Idx|Index|Slide|Cur)\(', t):
            bad.append((fp.name, "свой слайдер обложки — брать CardCover/useCoverSlider; "
                                 "иначе в приложении два разных перелистывания (ЗКН-К001)"))
    return bad


def check_b001():
    """ЗКН-Б001 — ИМЯ КНИГИ ЦЕЛОЕ. ПОЛОВИНА — НЕ ИМЯ.

    У книги имя составное: «Бхагавад-гита» + «как она есть». Показать только
    `titleLine1` — значит показать ПОЛОВИНУ: в одном месте книга зовётся полно,
    в другом обрублена.

    Читатель решит, что это РАЗНЫЕ книги.

    Законно: обе строки рядом (имя в две строки) ИЛИ `bookFullTitle()` (имя в
    одну). Нарушение — `titleLine1` БЕЗ пары.
    """
    bad = []
    for fp in sorted(SRC.rglob("*.tsx")):
        t = fp.read_text(encoding="utf-8")
        if "titleLine1" not in t:
            continue
        if "titleLine2" in t or "bookFullTitle" in t:
            continue          # имя целое — в две строки или через хелпер
        bad.append((fp.name, "показана ПОЛОВИНА имени книги (`titleLine1` без пары) — "
                             "читатель решит, что это другая книга (ЗКН-Б001)"))
    return bad


CHECKS = [
    ("ЗКН-Б001", "имя книги целое, не половина", check_b001),
