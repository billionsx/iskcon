...
ПРАВИЛО 3 — ЗВУК РИСУЕТ ОДИН КОМПОНЕНТ (ЗКН-Д015). Тег <audio> живёт ТОЛЬКО
  ...
  РАНЬШЕ У ЭТОГО ПРАВИЛА БЫЛО ИСКЛЮЧЕНИЕ — плеер пробной оболочки `x/`:
  оболочки никогда не работали вместе, развилка в `main.tsx` рендерила либо
  одну, либо другую, и «вторая реализация» там была не побочным эффектом, а
  целью. 21.07.2026 основатель снёс сначала плеер, следом всю оболочку —
  исключение исчезло вместе с предметом. Осталась ОДНА оболочка и, значит,
  ровно два законных места для <audio>. Послабление, пережившее свою причину,
  — это дыра: под тем же именем завтра встанет что угодно.
