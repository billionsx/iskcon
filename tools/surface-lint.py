...
ПРАВИЛО 3 — один плеер НА ОБОЛОЧКУ (ЗКН-Д015). Тег <audio> живёт ТОЛЬКО в ВКЗ
  (`AudioShowcaseCard.tsx`), в глобальном плеере (`player/store.tsx`) и в плеере
  пробной оболочки (`x/Player.tsx`). Любой второй <audio> ВНУТРИ одной оболочки —
  это вторая реализация звука: она разъезжается со стандартом и остаётся серой,
  когда стандарт уже вычищен (так и вышло с голосовыми).

  Почему пробная оболочка не считается нарушением. Закон боится РАСХОЖДЕНИЯ
  реализаций внутри работающего приложения. Оболочки `x/` и текущая никогда не
  работают вместе: развилка в `main.tsx` рендерит либо одну, либо другую, а
  правило 8 гейта Д028 запрещает им импортировать друг друга. Пробная оболочка
  и создана затем, чтобы заменить текущую целиком — «вторая реализация» здесь
  не побочный эффект, а цель. Внутри `x/` действует тот же запрет: второй
  <audio> там будет пойман счётчиком ниже.
...
AUDIO_OK = {"AudioShowcaseCard.tsx", "store.tsx", "Player.tsx"}
...
def main() -> int:
    ...
    for f in sorted(SRC.rglob("*.tsx")):
        ...
        # ПРАВИЛО 3 — второй плеер (внутри оболочки)
        ...
        # тот же запрет ВНУТРИ пробной оболочки: там звук тоже рисует один файл
        if f.parent.name == "x" and f.name != "Player.tsx" and AUDIO_TAG.search(text):
            bad.append(f"{rel} — ЗКН-Д015: второй плеер внутри пробной оболочки. "
                       f"Звук в `x/` рисует только x/Player.tsx")
