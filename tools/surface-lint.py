...
ПРАВИЛО 3 — ЗВУК РИСУЕТ ОДИН КОМПОНЕНТ (ЗКН-Д015). Тег <audio> живёт ТОЛЬКО
  в ВКЗ (`AudioShowcaseCard.tsx`) и в глобальном плеере (`player/store.tsx`).
  Любой второй <audio> — это вторая реализация звука: она разъезжается со
  стандартом и остаётся серой, когда стандарт уже вычищен (так и вышло с
  голосовыми).

  РАНЬШЕ У ЭТОГО ПРАВИЛА БЫЛО ИСКЛЮЧЕНИЕ — плеер пробной оболочки: оболочки
  `x/` и текущая никогда не работают вместе, развилка в `main.tsx` рендерит
  либо одну, либо другую. Исключение снято вместе с файлом: пробный плеер
  прожил свой срок, его замеры уехали в `player/`, и 21.07.2026 основатель
  распорядился его удалить. Послабление, пережившее свою причину, — это дыра:
  под тем же именем завтра встанет что угодно. Теперь в `x/` не может быть
  НИКАКОГО <audio>.
...
def main() -> int:
    ...
    for f in sorted(SRC.rglob("*.tsx")):
        ...
        # в пробной оболочке звука нет вовсе: её плеер удалён (см. ПРАВИЛО 3)
        if f.parent.name == "x" and AUDIO_TAG.search(text):
            bad.append(f"{rel} — ЗКН-Д015: <audio> в пробной оболочке. Свой плеер "
                       f"у `x/` удалён; звук рисует player/store.tsx")
