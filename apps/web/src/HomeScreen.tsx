...
function Figure({ src, ratio, pos, caption }: { src: string; ratio?: string; pos?: string; caption?: string }) {
  return (
    <figure style={{ margin: "16px 0 0" }}>
      ...
      {caption && <figcaption style={{ margin: "10px auto 0", maxWidth: 420, textAlign: "center", fontFamily: "var(--font-scripture)", fontStyle: "italic", fontSize: "var(--text-footnote)", color: "var(--color-label-3)", lineHeight: 1.45 }}>{caption}</figcaption>}
    </figure>
  );
}
...
const FORMS = [
  { t: "Мантра", d: "Звуковая форма Кришны и Шримати Радхарани (Харе). Повторение мантры Харе Кришна (тихо — джапа, громко — киртан) помогает очистить ум и сердце, мгновенно соединяя вас с Богом и позволяя ощущать Его присутствие.", to: "/kirtans" },
  { t: "Книги", d: "Литературная форма Кришны и Шримати Радхарани (Харе). Изучение священных писаний, таких как «Бхагавад-гита», «Шримад-Бхагаватам» и «Шри Чайтанья-чаритамрита», позволит вам встретить Бога на страницах этих текстов.", to: "/books" },
  { t: "Божества", d: "Проявленные формы Кришны и Шримати Радхарани (Харе) в этом мире неотличны от их вечных духовных форм, что позволяет преданным служить Им непосредственно.", to: "/" },
  { t: "Прасад", d: "Вегетарианская пища, предложенная Кришне и Шримати Радхарани (Харе), становится освящённой и передаёт духовную силу и любовь, помогая очищать ум и тело, а также устанавливать связь с Богом.", to: "/prasad" },
  { t: "Садху", d: "Святые, хранящие в своих сердцах Кришну и Шримати Радхарани (Харе), являются духовными наставниками и учителями, источниками вдохновения, показывающими живой пример преданности, служения и любви.", to: "/hero" },
  { t: "Дхама", d: "Святые места, неотличные от духовного мира, где обитают Кришна и Шримати Радхарани (Харе). Посещение Дхамы углубляет духовное сознание и помогает почувствовать близость к Богу. Примеры Дхамы — Вриндаван и Маяпур.", to: "/dhama" },
];
const PRINCIPLES = [
...
const PURPOSES = [
  ...
  "Преподавать и поощрять движение санкиртаны — коллективное пение святого имени Бога, как оно раскрыто в учениях Господа Гауранги Махапрабху.",
...
const SP_BIO_TAIL: string[] = [
  ...
  "Сегодня Шрила Прабхупада почитается как мировой ачарья — для всех вайшнавов, независимо от страны, культуры и языка, — как личность, исполнившая сокровенное желание Гауранги Махапрабху: распространить святое имя Кришны и путь чистой бхакти по всей Земле.",
];
...
const PARAMPARA: { name: string; note: string }[] = [
  ...
  { name: "Гауранга Махапрабху", note: "1486–1534 · Кришна и Шримати Радхарани в одном облике" },
  ...
  { name: "А.Ч. Бхактиведанта Свами Шрила Прабхупада", note: "1896–1977 · основатель ИСККОН" },
];
    ...
    <div role="dialog" aria-modal="true" aria-label="О Шриле Прабхупаде" onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "10px 22px 32px", fontFamily: "var(--font-text)" }}>
          ...
          <div style={{ marginTop: 26, fontFamily: "var(--font-display)", fontSize: "var(--text-body)", fontWeight: 700, letterSpacing: TR_TITLE, color: "var(--color-label)" }}>Жизнь</div>
          ...
          <div style={{ marginTop: 26, fontFamily: "var(--font-display)", fontSize: "var(--text-body)", fontWeight: 700, letterSpacing: TR_TITLE, color: "var(--color-label)" }}>Главные ценности</div>
  ...
  return (
    <div style={{ fontFamily: "var(--font-text)" }}>
      ...
      <Section id="hsec-today">
        ...
        <div style={{ marginTop: 26, fontFamily: "var(--font-display)", fontSize: "var(--text-body)", fontWeight: 700, letterSpacing: TR_TITLE, color: "var(--color-label)" }}>Храмы по всему миру</div>
                ...
                <button type="button" disabled={!tap} onClick={() => f.to && onOpenPath?.(f.to)}
                  ...
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontFamily: "var(--font-display)", fontSize: "var(--text-callout)", fontWeight: 600, letterSpacing: TR_TITLE, color: "var(--color-label)" }}>{f.t}</span>
                    ...
                    <span style={{ display: "block", marginTop: 4, fontFamily: "var(--font-text)", fontSize: "var(--text-footnote)", lineHeight: 1.5, letterSpacing: TR_BODY, color: "var(--color-label-2)" }}>{f.d}</span>
                  </span>
                </button>
              </li>
            );
      ...
      <Section id="hsec-dhama">
        <SectionHead eyebrow="Дхама" title="Святые места" subtitle="Дхамы — святые места, неотличные от духовного мира, где обитают Кришна и Шримати Радхарани. Их посещение углубляет духовное сознание и приближает к Богу." />
        <div style={{ display: "grid", gap: 12 }}>
          <PlaceCard src="/media/vrindavan.webp" title="Вриндаван" sub="Земля игр Кришны" />
          <PlaceCard src="/media/mayapur.webp" title="Маяпур" sub="Место явления Гауранги Махапрабху" />
        </div>
      </Section>
...
<SectionHead eyebrow="Ачарья-основатель" title="Шрила Прабхупада" subtitle="Выдающийся духовный учитель, принёсший сознание Кришны в западный мир и заложивший основы глобального вайшнавского возрождения. Его жизнь с любовью описана в семитомной «Шрила Прабхупада-лиламрите», читаемой по всему миру." />
        ...
        <button type="button" onClick={() => setSpOpen(true)}
          ...
          style={{ marginTop: 18, padding: "8px 2px", background: "none", border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4,
            fontFamily: "var(--font-text)", fontSize: "var(--text-subhead)", fontWeight: 600, letterSpacing: TR_BODY, color: "var(--color-gold-deep)", WebkitTapHighlightColor: "transparent" }}>
      ...
      <Section>
        <div style={{ padding: "26px 22px", ...fill, borderRadius: 22 }}>
          ...
          <div style={{ marginTop: 16, fontFamily: "var(--font-scripture)", fontStyle: "italic", fontSize: "var(--text-body)", lineHeight: 1.55, letterSpacing: TR_BODY, color: "var(--color-label)" }}>
        ...
        <div style={{ position: "relative", marginTop: 22, paddingLeft: 26 }}>
          ...
          {PARAMPARA.map((p, i) => (
            <div key={i} style={{ position: "relative", paddingBottom: i === PARAMPARA.length - 1 ? 0 : 22 }}>
              ...
              <div style={{ fontFamily: "var(--font-display)", fontSize: "var(--text-callout)", fontWeight: 700, letterSpacing: "-0.2px", color: "var(--color-label)", lineHeight: 1.25 }}>{p.name}</div>
              <div style={{ marginTop: 2, fontFamily: "var(--font-text)", fontSize: "var(--text-footnote)", color: "var(--color-label-3)", lineHeight: 1.35 }}>{p.note}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section id="hsec-next">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <NavCard mark={<MaskMark src="/bbt.svg" size={44} />} title="Книги" subtitle="Бхагавад-гита, Шримад-Бхагаватам и наследие ачарьев" onClick={() => onOpenPath?.("/books")} />
          <NavCard mark={<MaskMark src="/gauranga.svg" size={44} />} title="Киртаны" subtitle="Бхаджаны, молитвы и мантры" onClick={() => onOpenPath?.("/kirtans")} />
          <NavCard mark={<MaskMark src="/prabhupada.svg" size={44} />} title="Герои" subtitle="Господь, аватары и спутники" onClick={() => onOpenPath?.("/hero")} />
