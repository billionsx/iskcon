...
/** Прежние хосты, которые ЕЩЁ привязаны к воркеру и 301-ят на канонический.
 *  Пусто: gaurangers.com снят с проекта решением основателя (18.07.2026). */
export const ALIAS_HOSTS: readonly string[] = [];

/** Снятые с проекта хосты. Держим списком не для работы, а для ПАМЯТИ и гейта:
 *  снятый домен не должен вернуться в маршруты воркера случайно, а деплой пишет
 *  их состояние в `deploy_checks` — чтобы было видно, отвязались они на самом
 *  деле или всё ещё резолвятся. */
export const RETIRED_HOSTS: readonly string[] = ["gaurangers.com", "www.gaurangers.com"];
...
/** Служебные хосты, на которых редирект ЗАПРЕЩЁН: превью воркера, на котором
 *  деплой мгновенно проверяет свежесть бандла (301 сорвал бы проверку). */
export const isServiceHost = (host: string): boolean => host.endsWith(".workers.dev");
