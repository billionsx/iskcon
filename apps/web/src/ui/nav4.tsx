...
export function ScopeTitle({ items, active, onChange, ariaLabel = "Область", sticky = true }: {
  items: NavItem[]; active: string; onChange: (id: string) => void; ariaLabel?: string;
  /** ЗКН-Н019: Tier-2 — навигация, она НЕ уезжает. Липнет под Tier-1.
   *  Иначе клик по Tier-3 прокручивал страницу и прятал Tier-2 под шапку. */
  sticky?: boolean;
}) {
  const cRef = useRef<HTMLDivElement>(null);
  const iRefs = useRef<Record<string, HTMLButtonElement | null>>({});
  ...
  return (
    <nav ref={cRef} aria-label={ariaLabel}
      style={{ display: "flex", alignItems: "baseline", gap: 14, padding: "8px 0",
        overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch",
        ...(sticky ? {
          position: "sticky" as const, top: "var(--h-hall-tabs)", zIndex: 22,
          margin: "0 -16px", paddingInline: 16,
          background: "var(--color-glass-nav)", backdropFilter: "blur(40px) saturate(180%)",
...
export function FilterChips({ items, active, onChange, ariaLabel = "Фильтр", sticky = false, stickyTop, navRef }: {
  items: NavItem[]; active: string; onChange: (id: string) => void; ariaLabel?: string;
  /** ЗКН-Н019: якорная навигация длинной страницы липнет и не уезжает при скролле. */
  sticky?: boolean;
  /** Отступ сверху: ПОД чем липнуть. По умолчанию — под Tier-1 (ЗКН-Н010). */
  stickyTop?: string;
  /** Ссылка на сам <nav>. ВАЖНО: обёртка <div ref> вокруг липкого меню ЛОМАЕТ sticky —
   *  высота обёртки равна высоте меню, у sticky нет хода, и он уезжает вместе с ней. */
  ...
  return (
    <nav ref={(el) => { (cRef as { current: HTMLDivElement | null }).current = el as HTMLDivElement; navRef?.(el); }} aria-label={ariaLabel}
      style={{ display: "flex", gap: 7, padding: "12px 0", overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch",
        ...(sticky ? {
          position: "sticky" as const, top: stickyTop ?? "var(--h-hall-tabs)", zIndex: 20,
          margin: "0 -16px", paddingInline: 16,
          background: "var(--color-glass-nav)", backdropFilter: "blur(40px) saturate(180%)",
          WebkitBackdropFilter: "blur(40px) saturate(180%)",
