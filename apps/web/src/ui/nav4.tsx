...
export function FilterChips({ items, active, onChange, ariaLabel = "Фильтр", sticky = false }: {
  items: NavItem[]; active: string; onChange: (id: string) => void; ariaLabel?: string;
  /** ЗКН-Н019: якорная навигация длинной страницы липнет ПОД Tier-1 и не уезжает при скролле. */
  sticky?: boolean;
}) {
  const cRef = useRef<HTMLDivElement>(null);
  const iRefs = useRef<Record<string, HTMLButtonElement | null>>({});
  ...
  return (
    <nav ref={cRef} aria-label={ariaLabel}
      style={{ display: "flex", gap: 7, padding: "12px 0", overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch",
        ...(sticky ? {
          position: "sticky" as const, top: "var(--h-hall-tabs)", zIndex: 20,
