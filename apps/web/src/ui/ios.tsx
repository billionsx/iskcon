...
/**
 * Шеврон строки — путь 5.67 × 9.67, штрих 1.33 (📐 5.22 · 5.24).
 *
 * В стандарте лежали ДВА числа: 7 × 12 (ЗКН-Д018) и 5.67 × 9.67 (замер). Это не
 * противоречие, а один знак, померенный дважды: 5.67 × 9.67 — ПУТЬ, 7 × 12 —
 * ГАБАРИТ со штрихом (5.67 + 1.33 = 7.0, 9.67 + 1.33 = 11.0). Прежняя версия
 * попадала в габарит неверным разложением: путь 5.0 × 10.2 при штрихе 1.9 — то
 * есть 5.7 физических пикселя, а §5.22 говорит, что штрих квантуется и бывает
 * только 3 или 4 пикселя. Промежуточных значений нет.
 */
export function Chevron({ color = INK3 }: { color?: string }) {
  return (
    <svg width="7" height="11" viewBox="0 0 7 11" fill="none" aria-hidden
      ...
      <path d="M0.665 0.665 6.335 5.5l-5.67 4.835" stroke="currentColor" strokeWidth="1.33"
 ...
 *
 * В стандарте врезок ТРИ (§5.19): 32 под знаком, 44 под текстом без знака, 68.7
 * под плиткой иконки — врезка есть СЛЕДСТВИЕ строки, а не константа. У нас семья
 * схлопывается в одно число, и это не упрощение: ЗКН-Д024 убрал из строк иконки и
 * плитки вовсе, поэтому содержимое всегда начинается на 16, и все три случая
 * сводятся к одному. Вернётся иконка в строку — вернётся и семья.
 */
...
/**
 * Строка сгруппированного списка. 49px (одна линия, 📐 5.19) / 60px (с подписью, ⚠),
