...
/** Свернуть диакритику: «Kṛṣṇa» → «krsna».
 *
 * ЛОВУШКА, СТОИВШАЯ ВСЕЙ ДЕТЕКЦИИ: NFD разбирает «й» на «и» + КРАТКУЮ (U+0306).
 * Снимая ВСЕ надстрочные знаки, мы убивали «й»: fold("анитйам") → "анитиам".
 * Матчер искал «йа» и не находил НИКОГДА — вся ветка была мертва в проде.
 * Краткую (U+0306) не трогаем: она не диакритика транслитерации, она БУКВА.
 */
const DIA = /[\u0300-\u0305\u0307-\u036F\u0483-\u0489]/g;
function fold(w: string): string {
  return w.normalize("NFD").replace(DIA, "").normalize("NFC")
...
// «йо», «йу», «йе» стоят в РУССКИХ написаниях имён («Дурьйодхана»). Оставляем
// только однозначные слоги: русский пишет «я», шастра — «йа».
const TRANSLIT_CYR = /(йа|кша|джн|шча|сйа|нйа|тйа|рйа|хйа|дхй|бхй|ттв)/;
...
function verseRuns(words: string[]): Array<[number, number]> {
  ...
  while (i < words.length) {
    ...
    // не открывает: иначе имена в русском написании — «Дурьйодхана»,
    // «Дхриштадьюмна» — уехали бы в курсив как стихи. В настоящем стихе
    // диакритика стоит хотя бы на одном слове.
    if (cls[i] !== "strong") { i++; continue; }
    let a = i, b = i;
...
/** ЗКН-Д013 — СВЯТОЕ ИМЯ ЗВУЧИТ ВСЕГДА, даже посреди нашей прозы.
 *
 *  ⚠️ Объявление регулярки ПРОПАЛО при правке (осталось только использование) —
 *  и `renderTerms` падал на первом же вызове: краш в браузере, а не «замечание».
 *  Ловим связку из трёх и более имён — Харе · Кришна · Рама, кириллицей и латиницей. */
const MANTRA_RE = /(?:\b(?:Харе|Кришна|Кришнa|Рама|Hare|Kṛṣṇa|Krishna|Krsna|Rāma|Rama)\b[\s,·]*){3,}/gi;
 ...
 */
/* ─────────────────────────────────────────────────────────────────────────
 * СВЯТОЕ ИМЯ — ЭТО ПОВТОРЕНИЕ, А НЕ УПОМИНАНИЕ.
 *
 * ⚠️ ЗКН-Ф006 — здесь `MANTRA_RE` ВЫЗЫВАЛАСЬ, НО НЕ БЫЛА ОБЪЯВЛЕНА. Сборка это
 * пропускает (esbuild типы не проверяет), а браузер падает: ReferenceError в
 * `renderTerms` — а через неё идёт ВСЯ проза приложения (комментарии, жития,
 * карточки). Белая страница в проде, молча.
 *
 * Граница между мантрой и прозой — не словарь исключений, а ЧИСЛО ПОВТОРОВ:
 *
 *   «движение Харе Кришна»                              → проза (два слова)
 *   «Харе Кришна Харе Кришна Кришна Кришна Харе Харе»   → мантра (восемь)
 *
 * Четыре имени подряд не встречаются в речи о чём-то — они и есть воспевание.
 * Поэтому святое имя звучит (Georgia-курсивом) даже без единой диакритики.
 * ───────────────────────────────────────────────────────────────────────── */
/* ⚠️ ЗКН-С004 — `\b` В JS РАБОТАЕТ ТОЛЬКО ДЛЯ ЛАТИНИЦЫ.
 * `\b` определена через `\w` = [A-Za-z0-9_]. Между «Харе» и пробелом границы
 * для движка НЕТ — регулярка с `\b` молчит на ВСЕЙ кириллице. Проверено:
 * с `\b` ни одна маха-мантра не распознавалась (0/3). Границу слова в
 * кириллице даёт только `\p{L}` с флагом `u`. */
const MANTRA_RE = /(?:(?:Харе|Кришна|Рама)(?![\p{L}])[\s,]*){4,}/giu;
