...
type RefParts = { book: string; loc?: string; to?: string };

// Единый разбор шастрической ссылки: книга + локатор + (опц.) deep-link
// внутрь читалки. Поддерживает компактные коды (ШБ 1.3.28, ЧЧ Мадхья 20.4),
// БРС, БГ, и крупные книги без deep-link (Брахма-самхита, Кришна-сандарбха,
// Лагху-бхагаватамрита, Говинда-лиламрита, Гита-говинда).
// Все ref в данных приводятся к одному компактному виду без запятых.
function expandRef(ref: string): RefParts {
  const r0 = ref.trim();
  const r = r0.replace(/^[«"]+|[»"]+$/g, "").replace(/^Шри\s+/, "").trim();
  // Шримад-Бхагаватам — компакт и развёрнутый
  ...
  // Чайтанья-чаритамрита: ЧЧ Мадхья 20[.396]
  m = r.match(/^(?:ЧЧ|Чайтанья-чаритамрита)\s+(Ади|Мадхья|Антья)\s+(\d+)(?:\.(\d+))?$/);
  if (m) {
    const lila = m[1] === "Ади" ? "adi" : m[1] === "Мадхья" ? "madhya" : "antya";
    const loc = m[3] ? m[1]+" "+m[2]+"."+m[3] : m[1]+" "+m[2];
    const to = m[3] ? "/book/cc/"+lila+"/"+m[2]+"/"+m[3] : "/book/cc/"+lila+"/"+m[2];
    return { book: "Чайтанья-чаритамрита", loc, to };
  }
...
// Совместимость со старым именем (используется в одной точке для cite).
function expandCiteRef(ref: string): string {
  const p = expandRef(ref);
  return p.loc ? p.book + " " + p.loc : p.book;
}
...
function SourceLink({ parts, onNavigate, size = 12.5 }: { parts: RefParts; onNavigate?: (href: string) => void; size?: number }) {
  ...
  const inner = (
    <span style={{ display: "inline-flex", flexWrap: "wrap", columnGap: 5, rowGap: 0, alignItems: "baseline" }}>
      <span style={{ whiteSpace: "nowrap" }}>{parts.book}</span>
...
// byId) + ссылка-источник «где сказал» одной строкой; название книги и
// локатор не разрываются внутри. Если есть deep-link в читалке (ШБ/БГ/ЧЧ)
// — добавляется «›». Размер 12.5/lineHeight 1.4 — компактно, без переносов
// на мобильном для большинства ссылок.
