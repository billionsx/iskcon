/* /play — корневая оболочка клона Apple Music (iOS 26, тёмная тема).
...
const TAB_ICON: Record<Tab, (s?: number) => React.ReactNode> = {
  home: (s) => I.lib({ s }), new: (s) => I.mic({ s }), radio: (s) => I.radio({ s }),
  library: (s) => I.note({ s }), search: (s) => I.lib({ s }),
};
                ...
                <div className="amx-tabs">
                  {(["home", "new", "radio", "library"] as Tab[]).map((t) => (
                    <button key={t} className={"amx-tab" + (baseTab === t ? " on" : "")} onClick={() => setTab(t)}>
                      {TAB_ICON[t](26)}
                      <span>{t === "home" ? "Книги" : t === "new" ? "Лекции" : t === "radio" ? "Киртаны" : "Бхаджаны"}</span>
                    </button>
                  ))}
                </div>
