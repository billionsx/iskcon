...
function flattenDossierTopic(dos: PdfDossier | null, ptab: string, psub: string): { title: string; sub: string; rows: Array<[string, string]>; sections: PrintSection[] } | null {
  ...
  for (const sec of secs) {
     ...
     * она правит канонические имена (голая форма «Шри Чайтанья Махапрабху» →   lint-ok: комментарий НАЗЫВАЕТ снятую форму — как её называет сам cardText.ts (он в ALLOW по той же причине)
     * «Гауранга Махапрабху») и разбивает предложения, нанизанные через точку
     * с запятой.
     *
